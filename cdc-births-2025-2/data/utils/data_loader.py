...
def load_natality_data() -> pd.DataFrame:
    ...
    candidate_paths = [
        ...
        base_dir / "Data" / "Provisional_Natality_2025_CDC.xlsx",
        base_dir / "Provisional_Natality_2025_CDC.xlsx",
        Path.cwd() / "data" / "Provisional_Natality_2025_CDC.xlsx",
        Path.cwd() / "Provisional_Natality_2025_CDC.xlsx",
    ]
    ...
    # Fallback: search for any .xlsx file matching natality or cdc
    if file_path is None:
        for p in base_dir.rglob("*.xlsx"):
            if "natality" in p.name.lower() or "cdc" in p.name.lower():
                file_path = p
                break

    if file_path is None or not file_path.exists():
        expected_path = base_dir / "data" / "Provisional_Natality_2025_CDC.xlsx"
        raise FileNotFoundError(
            f"Data file not found at {expected_path}.\n\n"
            "⚠️ GitHub / Streamlit Cloud Diagnostic:\n"
            "The Excel workbook was likely not committed or pushed to your GitHub repository.\n"
            "To fix this:\n"
            "1. Check that 'data/Provisional_Natality_2025_CDC.xlsx' exists in your GitHub repository.\n"
            "2. If using git CLI, run:\n"
            "   git add data/Provisional_Natality_2025_CDC.xlsx\n"
            "   git commit -m 'Add natality dataset'\n"
            "   git push\n"
            "3. If using the GitHub web UI, navigate to the 'data' folder on GitHub (create it if needed) and upload 'Provisional_Natality_2025_CDC.xlsx'."
        )
